...
const EditItem = () => {
  ...
  const shopdetails = myShopData.find(shop => shop.items.some(item => item._id === id));

  const navigate = useNavigate("");

  const [name, setName] = useState(itemDetails?.name);
  const [frontendImage, setFrontendImage] = useState(itemDetails?.image);
  const [backendImage, setBackendImage] = useState("");
  const [price, setPrice] = useState(itemDetails?.price);
  const [category, setCategory] = useState(itemDetails?.category);
  const [foodType, setFoodType] = useState(itemDetails?.foodType);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/items/edit/${itemDetails._id}`, formData, { withCredentials: true });
      ...
      setLoading(false);
      navigate(`/all-item/${shopdetails._id}`);

    } catch (error) {
      setLoading(false);
      toast.error(error.response?.data?.message || "Edit Item failed");
    }
      ...
      <main className='flex flex-col items-center justify-center p-6 bg-gradient-to-br from-orange-50 to-white min-h-screen'>
        <div className="w-full max-w-lg bg-white shadow-xl rounded-md p-5 border border-orange-100">
          <div className="icon-div-upper flex items-center md:gap-[38%] gap-[30%]">
            <i onClick={() => navigate(`/all-item/${shopdetails._id}`)} className="ri-arrow-left-long-line text-2xl cursor-pointer active:scale-95"></i>
            <FaUtensils className='text-[#ff4d2d] w-16 h-16 mb-4 bg-orange-100 p-4 rounded-full'/>
          </div>
          <h1 className='font-semibold text-gray-900 text-xl'>Edit Food Item</h1>
          <form className='space-y-4'>
            ...
            <button onClick={handleSubmit} className='bg-[#ff4d2d] text-white px-5 py-2 mt-5 rounded-md w-full uppercase cursor-pointer active:scale-95 transition-all hover:bg-[#e64323]' disabled={loading}>
              {loading ? <ClipLoader size={20} color='white' /> : "edit item"}
            </button>
          </form>
        </div>
      </main>
